...
class HealthChecker:
    ...
    def __init__(self) -> None:
        self._checks: dict[str, Callable[[], asyncio.Awaitable[CheckResult]]] = {}

    def register(
        self, name: str, check_fn: Callable[[], asyncio.Awaitable[CheckResult]]
    ) -> None:
        """注册一个健康检查函数。"""
    ...
    async def _run_check(
        name: str, fn: Callable[[], asyncio.Awaitable[CheckResult]]
    ) -> CheckResult:
        start = time.time()
        try:
            result = await fn()
            ...
            return CheckResult(
                name=name, status="fail", detail=str(e),
...
def build_default_health_checks(settings: Any) -> HealthChecker:
    ...
    async def _check_registry() -> CheckResult:
        ...
        if len(tools) >= 3:
            return CheckResult(name="registry", status="ok", detail=f"{len(tools)} tools registered")
    ...
    async def _check_sandbox() -> CheckResult:
        ...
        try:
            ...
            req = ExecRequest(cmd="echo ok", cwd=Path.cwd(), env={}, timeout=5, profile=SandboxProfile.DANGER_FULL)
            result = await ex.run(req)
            if result.ok:
                return CheckResult(name="sandbox", status="ok", detail=f"mode={settings.sandbox.mode}")
            return CheckResult(name="sandbox", status="degraded", detail=result.error or "unknown error")
        except Exception as e:
