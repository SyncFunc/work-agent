...
- **接口签名**：`SandboxProfile`（str Enum 三值）、`ExecRequest`/`ExecResult` dataclass、`Executor` Protocol（`name` + `async run`）、`build_executor(mode, *, workspace, profile)`、`get_executor()` 模块级注入点、`FakeExecutor`（测试）。
- **OS 降级铁律**：内核隔离（landlock/seccomp/seatbelt）**可用时才用**，不可用时退化为进程级 + 告警，**绝不抛异常中断 Agent**。`unshare -n` 是 Linux 断网的最低成本手段。
- **Windows 现实**：无 seccomp/Landlock，`LocalExecutor` 内装 `CommandFilter` 应用层主动拦截越界写/联网/破坏性命令（**不打印告警**）；它弱于内核沙箱（可被混淆绕过）；真隔离靠 `docker`/`external`（WSL 属 Linux，landlock 可用）。
- **可注入**：`bash` 不直接 `subprocess`，而是经 `get_executor()`；测试替换为 `FakeExecutor`，避免联网/root。
- **对 M2.4 约束**：`bash` 工具将改为构造 `ExecRequest` 并调 `get_executor().run(...)`，把 `ExecResult` 转 `ToolResult`。
