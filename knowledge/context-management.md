...
> 主题：工具结果（tool_result）到底「保存」还是「临时注入」？结合 Claude Code 方案调研，给出本项目（work-agent）的结论与落地路线。
> 状态：M1 阶段为**设计结论 + 第 0 层已落地**；完整压缩层属 **M4 上下文与记忆里程碑**（采用 Claude Code 方案，详见 `knowledge/claude-code-context-management.md`）。

---

## 1. 核心结论：工具结果 = 对话历史（既保存、也注入）

「保存」 vs 「临时注入」是一个**伪二选一**。Claude Code 的实践如此：
...
| `conv` / `Session.messages`（上下文窗口投影） | 逐轮喂给模型的历史 | 随轮累积；`loop.py:242-246` 把 `Message(role="tool", content=res.output, tool_call_id=tc.id)` 进 `conv` | **未来可压缩**（M4） |
...
在完整压缩层（M4）之前，M1 已内置三道低成本防护，避免大输出直接撑爆上下文：
...
## 4. 压缩方案：采用 Claude Code 四层渐进防线

> 详细调研与 mermaid 流程图见 `knowledge/claude-code-context-management.md`。本项目**采用 Claude Code 方案**，不引入 Codex 的 ContextManager 体系。

Claude Code 的压缩是「从零成本到花一次模型调用」的渐进防线，**能不调模型就不调**：
...
| 3a | Session Memory Compact | 0 | 复用后台持续维护的「摘要文件」替代历史（零成本首选）。摘要按 **10 段固定结构**维护（Session Title / Current State / Task / Files / Workflow / Errors / Codebase / Learnings / Key results / Worklog，每段 ≤2K、整文件 ≤12K tokens）；增量提取触发阈值 `init 10K` / `between 5K` / `tool 3`；压缩时即时复用（详见 `claude-code-context-management.md` §3.5） |
| 3b | AI 摘要压缩 | 1 次调用 | LLM 生成 9 段结构化摘要（意图/技术概念/文件与代码/错误修复/解决思路/用户消息/待办/当前工作/下一步），第 9 段要求原文锚点 |
| 4 | Reactive Compact | 1 次调用 | API 返 413 时紧急重试 |

- **触发**：接近上限自动触发。阈值 = 有效窗口 − `AUTOCOMPACT_BUFFER_TOKENS`(~13K)；默认 200K 窗口→阈值 ~187K，1M 窗口→ ~967K。
- **防漂移**：压缩后**主动重新读取最近操作的文件最新版**贴回上下文，确保不「忘记」正在编辑的代码。
- **边界标记**：Compact Boundary 记录压缩点，下次只从边界后取消息。
- **固定底座**：AGENTS.md / 自动记忆 / 技能体从磁盘重新注入，永不进摘要（对抗长期衰减）。

**本项目落地映射（落到 `agent/context/`）**：

- **先做 Microcompact（0 成本）**：操作 `conv` 中较旧的 `role="tool"` 消息，把 `content` 替换为占位符，保留最近 N 个；不动 `tool_use`/`tool_result` 配对。
- **再做 Auto Compact（1 次调用）**：越阈值时把边界前历史整批压成 9 段结构化摘要，插入 Compact Boundary；压缩后重读最近文件最新版防漂移。
- **演进项**：后台增量维护「会话摘要文件」，压缩时优先复用（对应 3a Session Memory Compact，零成本）。
...
读大文件 / 长 bash 日志等超大输出：**不塞主上下文**，委托给**子代理**（M5 subagent，隔离上下文窗口），主窗口只回收「摘要 + 小元数据尾迹」。这与「能力正交：Tool/Skill/Subagent 三层」「Subagent 隔离上下文」的决策一致。

---

## 7. Token 计量与预算（已有基础）

- `_estimate_tokens`（`agent/cli.py`）已提供粗估；M4 应在 `ContextManager` 类组件里对 `conv` 做精确/启发式计量，输出分类明细（系统提示 / 动态对话 / 工具结果）。
...
3. Auto Compact：阈值触发 → 9 段结构化摘要 + Compact Boundary；压缩后重读最近文件防漂移。
4. prompt caching：稳定前缀（system / AGENTS.md 类）走缓存，降成本。
5. 子代理隔离接入（M5 协同）：大输出不外溢主窗口。
...
- 详细调研：`knowledge/claude-code-context-management.md`（Claude Code 四层压缩防线、阈值公式、9 段摘要、配对铁律、防漂移、双轨映射、M4 落地建议；含 DeepWiki 官方 / cyeam 源码拆解 / 掘金·finisky 上下文工程专栏等来源）。
- 《CLAUDE CODE 上下文管理机制深度解析》（blog.cyeam.com，2026-06-03）：messages 累积、Microcompact 占位符、Auto Compact 9 段摘要、配对约束、防漂移、阈值公式（有效窗口−13K）。
...
- 本项目：`CODEBUDDY.md`（双轨/上下文稀缺/能力正交）、`knowledge/INDEX.md`、`agent/core/events.py`（EventStream）、`agent/core/loop.py:242-246`（tool_result 进 conv）、`agent/runtime/registry.py`（`_cap_result` / `max_tool_output_chars`）、`agent/tools/fs.py`（`read` 分页 + `grep`）。
