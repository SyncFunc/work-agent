...
- **session 切换语义**：daemon 多会话常驻，前端 `attach` 其一，切换 = detach+attach；每会话定长环形缓冲（`deque(maxlen=K)`，建议 200）支撑回放、防内存膨胀；**回放缓冲仅收非 `transient` 持久化事件**（修复点②），瞬时 `tool_call_delta` 只实时转发不进缓冲，避免切换回放时参数预览重复；后台子 agent（`Session._bg_tasks`）在无人 attach 时由 daemon 单循环持续驱动，事件入缓冲、attach 后回放。
- **复用约定**：客户端渲染直接复用 `TerminalTransport._on_event(Event.from_dict(...))`（`TerminalTransport` 零改动，客户端不调其 `bind`）；命令分发抽为共享 `dispatch_command`（cli 与 daemon 共用，避免重复实现 `/context` `/compact` `/plan` 等语义）。
- **依赖/安全**：实现时向 `pyproject.toml` 增 `websockets`（仅 daemon/client 路径 import，run/chat 不受影响）；daemon 仅绑 `127.0.0.1`。
- **与既有重构衔接**：本设计是 `milestones/M-refactor-统一传输层与事件线格式.md` 的落地化——该文档断言「再实现 `WebTransport` 在 `bind` 里 `stream.subscribe(lambda ev: ws.send(ev.to_dict()))` 即可转发」；本设计进一步把 HITL 也经 ws 请求/应答闭环，形成完整 client-server 架构。
- **对 M6 衔接**：守护进程常驻 + sqlite 会话（`session_id`）天然支撑「会话恢复 / 可恢复可观测可测」（M6 生产化）的远程接入形态。
...
- **M6.2 已完成**（2026-07-22）：会话恢复落地。`agent/context/session_recovery.py` 新增 `rebuild_messages(stream)`（事件→OpenAI 消息：USER→user、DECISION→assistant(tool_calls)、TOOL_RESULT→tool、CLARIFY/PLAN→合成 tool 消息完成配对；FINAL/TEXT/TOOL_USE 忽略）+ `detect_interruption(stream)`（存在 TOOL_USE 缺 TOOL_RESULT → 中断）。`Session.from_store(...)` 类方法：load 完整 EventStream→rebuild→注入 messages + 持有 `event_stream`（供 daemon 播种回放缓冲）。中断处理：丢弃悬空 assistant 并注入 user「继续」提示，绝不出现悬空 tool_calls（呼应 M1.5 配对铁律）。CLI 新增 `resume <id>` / `fork <id>` 命令与 REPL `/resume <id>` `/fork <id>`（rebind session 局部变量）。daemon `SessionRegistry.restore_factory`+`restore()` 实现冷启动：attach 到内存缺失但 sqlite 存在的 id 时自动恢复并播种 `event_buffer`。**关键设计**：新增 `EventType.USER`（`loop.run` 每轮开头 emit），使 EventStream 成为完整可重放转录（重建 messages 与崩溃前一致）。fork 恢复无特殊分支（子 events 含父前缀拷贝，load→rebuild 与普通会话一致）。测试 `tests/test_session_recovery.py`（7 例，含跨重启 resume/中断/fork）+ `test_daemon.py` 冷启动恢复（1 例）；全量 `pytest` **392 passed** 无回归。详见 `milestones/M6-生产化/6.2-会话恢复流程.md`。
- **M6.3 已完成**（2026-07-22）：测试金字塔落地。① 三层目录：`tests/unit`(纯逻辑 mock) / `tests/integration`(含 daemon+schema 快照+tool tapes，确定性不调真 LLM) / `tests/e2e`(慢非确定，全 `@pytest.mark.slow`)；各层带 `__init__.py`。② `pyproject.toml` 加 `addopts = "-m 'not slow'"` + `markers=[unit/integration/e2e/slow]`（默认跑非 slow，e2e 走 nightly/手动）。③ 新增 `agent/testing/recorded_model.py`（`RecordedModel`：给定 `list[Decision]` FIFO 回放，Tier2 主力替身，区别于 `FakeModel` 轮转语义）。④ 新增 `tests/conftest.py`（`make_registry`/`settings_factory` 共享 fixture，echo/slow 两工具）。⑤ prompt 快照 Tier1：`tests/integration/test_prompt_snapshot.py` 渲染 `_build_system_parts(Settings())` 静态段为 JSON，首跑写基线 `tests/integration/fixtures/prompts/system_static.json` 并 skip，后续 diff，意外改动即失败（有意变更则删基线重建）。⑥ tool tapes Tier2：`tests/integration/test_tool_tapes.py` 断言工具顺序+参数 + 错误分支(超时/降级→`ToolResult(ok=False)` 不崩)。⑦ e2e Tier3：`tests/e2e/test_e2e_resume_fork.py::test_e2e_resume_then_fork_across_restart` 跨 resume→fork→继续断言父前缀保留。⑧ **⚠️ 落盘 seq 全局性坑**：`SessionStoreSink` 落盘必须用「会话级全局 seq」覆盖 per-run seq（新 run `EventStream` 从 0 起；否则 resume/fork 后新事件经 `INSERT OR REPLACE` 主键 `(session_id,seq)` 覆盖父前缀）；修复见 `session_store.py` 的 `_next_seq()`+`SessionStoreSink.__call__`（惰性取 MAX(seq)+1 覆盖）。验证：`pytest -q` **395 passed**（含 e2e 基线首建 skip 1），`pytest -m slow` 1 passed。详见 `milestones/M6-生产化/6.3-测试分层重构.md`。
- **M6.4 已完成**（2026-07-22）：CI 工作流落地。新增 `.github/workflows/ci.yml`：① `fast` job（push/PR 门禁）= `ruff check .` → `ruff format --check .` → `basedpyright` → `pytest -q`（默认 `-m 'not slow'`）→ `pytest --cov=agent --cov-report=xml -q`，矩阵 Python 3.12，`concurrency` 取消旧跑；② `slow` job 仅 `schedule`(nightly `0 3 * * *`)/`workflow_dispatch` 触发 `pytest -m slow`，注入 `AGENT_LLM_API_KEY` secret，不阻塞 PR。配套 `pyproject.toml` dev 补 `pytest-cov/ruff/pytest-xdist`，加 `[tool.ruff]`(select E,F,W,I,UP,B；ignore E501；`known-first-party=["agent"]`)、`[tool.ruff.format]`、`[tool.coverage.run|report]` 配置。落地时把全仓纳入 ruff 风格（60 文件 `ruff format`）并修掉一批被门捕获的真实问题：5 个 `(str,Enum)→StrEnum`；`bridge.py` 补 `TYPE_CHECKING` 下 `SessionHandle` 导入（修 F821 真实 bug）；`health.py` `asyncio.Awaitable→collections.abc.Awaitable` + `results` 类型 `list[CheckResult|BaseException]` + `isinstance(r, BaseException)` 收窄（basedpyright 0 errors）；`registry.py` `SessionHandle.name: str|None`；`session_recovery.py` `pending.pop` 加 None 守卫；`subagent.py` `# ruff: noqa: E402`（刻意延迟导入避免循环依赖）；`settings.py` 导入上移；`cli.py` `raise ... from None`；`loop.py`/`session.py` `zip(..., strict=True)`；`test_registry.py` `assert False→raise AssertionError`。验证：本地 `ruff check`/`ruff format --check`/`basedpyright agent/` 全绿，`pytest --cov` 395 passed，e2e 1 passed。详见 `milestones/M6-生产化/6.4-CI工作流.md`。
- **M6.5 已完成**（2026-07-22）：端到端验收落地。新增两条 e2e（均 `@pytest.mark.slow`）：`tests/e2e/test_recovery_e2e.py`（`test_resume_full_session_across_daemon_restart`：跑完→新建 `SessionStore(db)`+`Session.from_store` 模拟 daemon 重启→`messages` 与崩溃前一致→续跑成功；`test_resume_interrupted_turn_across_daemon_restart`：人工写入「DECISION(tool_calls)+TOOL_USE 无 TOOL_RESULT」模拟崩溃→重启→`detect_interruption` 为 True→重建注入「继续」→续跑产出最终答案）；`tests/e2e/test_fork_e2e.py`（`test_fork_across_daemon_restart_lineage`：fork→重启父子各自 `from_store`→子消息以父前缀开头；`list_sessions` 显 `parent_session_id` 血缘；父子续跑互不串台）。**「daemon 重启」忠实模拟**= 内存 Registry/Session 全丢、仅 SQLite `SessionStore` 保留 → e2e 用「新 `SessionStore(同db)`+`Session.from_store`」复现冷启动（与 `start_daemon` 一致），确定、快、不 flaky，无需 OS 子进程。**README 用法补充**：`resume <id>`/`fork <id>` 命令 + `chat` REPL `/resume`/`/fork` + `client --resume`；里程碑表 M6→✅。验证：本地 `pytest -q` 395 passed（全量回归零）、`pytest -m slow` 4 passed、CI `fast` job 全绿。详见 `milestones/M6-生产化/6.5-端到端验收.md`。

## M6 里程碑总结（生产化，2026-07-22 全部落地）

> 五步：M6.1→M6.2→M6.3→M6.4→M6.5，每步独立 commit+push；全量 `pytest -q` 395 passed、`pytest -m slow` 4 passed、CI(`ruff`+`basedpyright`+`pytest`+`cov`) 全绿。

- **会话恢复基础设施（M6.1/M6.2）**：以 `EventStream`(seq 单调、`transient` 不落盘) 为单一事实来源，永不压缩。`SessionStore`(sqlite `sessions`+`events` 表) + `SessionStoreSink`(订阅事件流零侵入持久化)；`Session.from_store` 从持久化重建 `messages` + 持有 `event_stream`；`rebuild_messages`(USER→user、DECISION→assistant(tool_calls)、TOOL_RESULT→tool、CLARIFY/PLAN→合成 tool 消息完成配对)；`detect_interruption`(TOOL_USE 无 TOOL_RESULT→注入「继续」)；fork = 复制父 events 前缀 + 记 `parent_session_id`，恢复路径无特殊分支。CLI `resume <id>`/`fork <id>` + REPL `/resume`/`/fork`；daemon `restore_factory`+`restore()` 冷启动播种 `event_buffer`。⚠️**落盘 seq 全局性坑**：`SessionStoreSink` 必须用会话级全局 seq 覆盖 per-run seq（新 run `EventStream` 从 0 起，否则 `INSERT OR REPLACE` 主键 `(session_id,seq)` 覆盖父前缀）。
- **测试金字塔（M6.3）**：`tests/unit`(mock LLM) / `tests/integration`(daemon+schema 快照+tool tapes，确定性) / `tests/e2e`(slow 非确定)；`pyproject` `addopts="-m 'not slow'"` + markers；`RecordedModel` 录制重放；prompt 快照(Tier1) + tool tapes(Tier2) + e2e(Tier3)。**录制闭环（已落地）**：`agent/testing/recorded_model.py` 提供 `decisions_from_eventstream(stream)`（从一次运行的 `EventStream` 提取 `DECISION` 决策序列）+ `dump_tape/load_tape`（决策序列 ↔ JSON tape 文件）+ `RecordedModel.from_tape(path)`（由 tape 文件构造回放）；`tests/integration/test_tool_tapes.py` 的 `test_record_replay_roundtrip` 验证全链路等价、`test_real_tape_file_replays` 从 `tests/integration/fixtures/tapes/echo_one.json` 真实回放；nightly 真实 LLM 录制的 tape 结构同此。**录制入口（M6.3 补强）**：`python -m agent.cli run <task> --record <path>`（`agent/cli.py`）在真实运行时落盘真实录像；nightly slow 通道 `tests/e2e/test_tape_record_slow.py::test_record_replay_autosaved_in_session_loop` 走完整 Session 链路自动 `dump_tape`+回放校验等价。真实录像（回放）主要在确定性场景消费：每次 commit 用 `RecordedModel.from_tape` 回放 nightly 录下的 tape 做回归守门、复现 bug、对比不同版本/模型的工具行为。
- **CI（M6.4）**：`.github/workflows/ci.yml` 快慢分离（`fast`=ruff+basedpyright+pytest+cov，`slow`=nightly e2e）；全仓纳入 ruff 风格并修真实 lint/类型问题（5×StrEnum、`SessionHandle` 导入、`Awaitable`/`BaseException` 收窄等）。
- **端到端验收（M6.5）**：resume 跨重启 + fork 跨重启 + 中断恢复，均以「新 `SessionStore`+`from_store`」忠实模拟 daemon 冷启动；README 补充 `resume`/`fork` 用法。
- **跨 M6 不变量**：① 恢复基于完整未压缩 `EventStream`；② 重建 `messages` 满足 OpenAI/DeepSeek 配对（呼应 M1.5）；③ 真实 LLM 调用只进 `slow`/e2e，绝不进 PR 门禁。
...
- **CLI 美化方案调研**：`knowledge/调研-CLI美化方案.md` —— 在「渲染层与 runner 解耦」架构**不变**前提下的终端观感提升选项。现状：已用 `rich`+`prompt_toolkit` 事件驱动渲染，缺口=颜色/边框/图标硬编码、工具参数 JSON 无高亮、长工具无 spinner、HITL 状态塞 prompt 前缀、无无 emoji 模式。**推荐短期增量（不碰 core/EventStream）**：① 统一主题 `ui/theme.py`+`settings.ui.theme`（Catppuccin/Gruvbox/plain，plain 关 emoji）；② 工具参数改 `Syntax(json/bash)` 高亮 + 统一 `box=ROUNDED`；③ 长工具 `Console.status` spinner；④ ptk 底部工具栏 + 斜杠命令补全。中期若做全屏 chat 再评估 **Textual**（须保留 headless/WebTransport 路径供 CI+daemon 客户端）。**不推荐** PyTermGUI / questionary（已被 ptk 覆盖）/ 引入 Go Lipgloss。
- **CLI 架构级重构调研（Textual 全屏 TUI）**：`knowledge/调研-Textual全屏CLI重构方案.md` —— 用户要求**架构级重构对标 Claude Code**、且**旧 CLI 先保留、重构完再迁移**。结论：选 **Textual**（Rich 同团队，声明式+TCSS+自带 asyncio loop+可跑浏览器，Python 界对应 Ink/React），**不是**自研双缓冲/PyTermGUI/ptk 全屏。做法=**新增一个与 `TerminalTransport` 平级的 `AgentTransport` 实现（`TextualTransport`）+ 全屏 `App`**，`AgentTransport`/`EventStream`/loop/daemon 协议**一字不改**；旧 `run`/`chat`/`client` 全保留，仅加 `chat --tui`。**最难=事件循环集成**：Textual 自带 loop 不能内层 `asyncio.run`；推荐把 `session.step` 跑成 **thread worker**（独立 loop），`TextualTransport` 用 `app.call_from_thread` 桥接事件、`ModalScreen`+线程安全 `Future` 做 HITL。测试用 `run_test()`+`Pilot` headless（CI 不破），`textual` 放 `tui` extra。迁移分 Phase0 依赖→Phase1 并行实现→Phase2 验证→Phase3 切默认。里程碑拆为 **M8-CLI全屏重构**（M8.0~M8.7）。被 M8 引用。
...
- **M8.7 迁移切换（chat 默认 TUI）**：`agent/cli.py` 的 `chat` 移除 `--tui`，改加 `--legacy`；新增 `_chat_should_use_tui(legacy)` 返回 `(not legacy) and sys.stdin.isatty()`。`use_tui=True`→`ChatApp(...).run()`（Textual 全屏）；否则旧 `TerminalTransport`+`_chat_repl`。**非 TTY 自动退回旧渲染**（CI/CliRunner 不卡死、旧单测绿）。`run`/`client` 本期不动。旧实现（`TerminalTransport`/`_chat_repl`/`client` 旧渲染）**保留**供 `--legacy` 回退，删除留观察期。测试：`test_chat_default_enters_tui_when_tty`（mock `_chat_should_use_tui→True` 断言 TUI 命中）+ `test_chat_legacy_flag_uses_old_repl`（断言 TUI 未用）。`README.md`/`CODEBUDDY.MD` 已同步。详见 `milestones/M8-CLI全屏重构/M8.7-迁移切换.md`。
- **M8.8 真机验收修复（6 项交互问题）**：
  1. **输入 `/` 内联命令提示**：`ChatApp` 新增 `ChatInput(TextArea)` 子类，重写 `_on_key` 拦截 enter/esc/up/down（仅当下拉 `_hint_open` 时生效，否则退回 TextArea 默认）；`on_text_area_changed` → `_update_hints()` 按前缀过滤 `_command_candidates()`（静态 `_HELP_ROWS` + 动态 `/skill <name>`），下拉 `#cmd_hint` 显示，Enter 接受填充输入框、方向键移动、点击项接受。
  2. **Ctrl+P 命令目录**：`AgentCommandProvider._COMMANDS` 首项 `📑 命令目录`，选中调 `_show_command_directory` → 复用 `/help` 在主区列出全部命令。
  3. **滚动卡顿（写入工具时）**：根因是单条超大结果/diff 的 `Syntax` 部件过重 + 旧 ToolBlock 把结果挂成 Collapsible 直接子节点导致无法折叠。`widgets._truncate(text, 6000)` 截断超大内容；自动吸底保持条件式（仅底部才 `scroll_end`）。
  4. **工具块无法折叠**：旧 `ToolBlock.compose` 覆盖了 `Collapsible` 的 title/Contents 结构导致 `-collapsed` 规则失效。改为把参数作为 Collapsible 子部件传入（进 `Contents`），结果也 `mount` 进 `Collapsible.Contents`（参数/结果分离、整体随折叠显隐）；加 `can_focus` + `space`/`enter` 折叠快捷键。
  5. **参数/结果分离**：参数 `Static(Syntax(json))` 与结果 `Static(Syntax(diff)/Markdown)` 分别带 `border_title`「参数」「结果」，同在 `Contents` 内。
