...
- **M8.8 真机验收修复（/ 帮助 + 滚动）**：① `dispatch_command` 新增 `/` 与 `/help` 分支，输出 `_HELP_ROWS` 全部 slash 命令清单（之前裸 `/` 报"未知命令"，与欢迎语"输入 / 查看命令"矛盾）。② TUI 真机滚动：TextArea 已占用 `up/down/pageup/pagedown`，故 `ChatApp` 新增 `BINDINGS` `ctrl+up`/`ctrl+down` → `action_scroll_log_up/down`（用 `_log.scroll_relative`）。③ **自动吸底改为条件式**：`_mount` 只在用户已停在底部（`scroll_offset.y >= max_scroll_y-1`）时才 `scroll_end`，否则保留其浏览历史的滚动位置（否则新消息持续把视图拽回底部，无法回看）。欢迎语同步提示 `Ctrl+↑/↓ 浏览历史` + `输入 / 或 /help 查看命令`。`Static` 取内容用 `.content` 属性（返回 `Text`，`.plain` 取纯文本），不是 `.renderable`。测试：`test_slash_help_lists_commands`/`test_slash_command_dispatched`、`test_scroll_log_and_preserve_position`/`test_autoscroll_when_at_bottom`。
