...
- **测试金字塔（M6.3）**：`tests/unit`(mock LLM) / `tests/integration`(daemon+schema 快照+tool tapes，确定性) / `tests/e2e`(slow 非确定)；`pyproject` `addopts="-m 'not slow'"` + markers；`RecordedModel` 录制重放；prompt 快照(Tier1) + tool tapes(Tier2) + e2e(Tier3)。
